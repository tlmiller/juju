...
func (st *RootKeyState) GetKey(ctx context.Context, id []byte, now time.Time) (macaroon.RootKey, error) {
	db, err := st.DB()
	if err != nil {
		return macaroon.RootKey{}, interrors.Capture(err)
	}
	key := rootKeyID{ID: id}
	getKeyStmt, err := st.Prepare("SELECT &rootKey.* FROM macaroon_root_key WHERE id = $rootKeyID.id", rootKey{}, key)
	if err != nil {
		return macaroon.RootKey{}, interrors.Errorf("preparing get root key statement %w", err)
	}
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if database.IsErrNotFound(err) {
			return interrors.Errorf("key with id %s %w", string(id), macaroonerrors.KeyNotFound)
		}
		return err
	})
...
`
	...
	if err != nil {
		return macaroon.RootKey{}, interrors.Errorf("preparing get latest key statement %w", err)
	}
...
func (st *RootKeyState) InsertKey(ctx context.Context, key macaroon.RootKey) error {
	...
	if err != nil {
		return interrors.Errorf("preparing insert root key statement %w", err)
	}
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		err := tx.Query(ctx, insertKeyStmt, row).Run()
		if database.IsErrConstraintPrimaryKey(err) {
			return interrors.Errorf("key with id %s %w", string(key.ID), macaroonerrors.KeyAlreadyExists)
		}
		return err
	})
	return interrors.Capture(err)
...
func (st *RootKeyState) removeKeysExpiredBefore(ctx context.Context, tx *sqlair.TX, cutoff time.Time) error {
	...
	if err != nil {
		return interrors.Errorf("preparing remove expired root key statement %w", err)
	}
