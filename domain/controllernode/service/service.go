...
func (s *Service) CurateNodes(ctx context.Context, toAdd, toRemove []string) error {
	...
	return interrors.Errorf("curating controller codes; adding %v, removing %v %w", toAdd, toRemove, err)
}
...
func (s *Service) UpdateDqliteNode(ctx context.Context, controllerID string, nodeID uint64, addr string) error {
	...
	return interrors.Errorf("updating Dqlite node details for %q %w", controllerID, err)
}
...
func (s *Service) IsKnownDatabaseNamespace(ctx context.Context, namespace string) (bool, error) {
	if namespace == "" {
		return false, interrors.Errorf("namespace %q is %w, cannot be empty", namespace, coreerrors.NotValid)
	}

	ns, err := s.st.SelectDatabaseNamespace(ctx, namespace)
	if err != nil {
		if !interrors.Is(err, controllernodeerrors.NotFound) {
			return false, interrors.Errorf("determining namespace existence %w", err)
		}
	}
