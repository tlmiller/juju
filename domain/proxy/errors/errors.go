...
const (
	...
	ProxyInfoNotSupported = interrors.ConstError("proxy info not supported")
	// ProxyInfoNotFound describes an error that occurs when a proxy information
	// cannot be found.
	ProxyInfoNotFound = interrors.ConstError("proxy info not found")
	// ProxyNotSupported describes an error that occurs when the underlying
	// provider does not support the proxy.
	ProxyNotSupported = interrors.ConstError("proxy not supported")
)
