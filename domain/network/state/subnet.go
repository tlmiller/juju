...
FROM   subnet`, Subnet{})
	if err != nil {
		return nil, interrors.Errorf("preparing select subnet statement %w", err)
	}
...
func (st *State) UpsertSubnets(ctx context.Context, subnets []network.SubnetInfo) error {
	...
	return db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		for _, subnet := range subnets {
			...
			if err != nil && !interrors.Is(err, coreerrors.NotFound) {
				return interrors.Capture(err)
			}
...
VALUES ($ProviderNetworkSubnet.*)`, providerNetworkSubnet)
	...
	if err := tx.Query(ctx, insertSubnetProviderIDStmt, providerSubnet).Run(); err != nil {
		if internaldatabase.IsErrConstraintPrimaryKey(err) || internaldatabase.IsErrConstraintUnique(err) {
			...
			return interrors.Errorf("provider id %q for subnet %q %w", subnetInfo.ProviderId, subnetUUID, coreerrors.AlreadyExists)
		}
		st.logger.Errorf("inserting provider id %q for subnet %q, %v", subnetInfo.ProviderId, subnetUUID, err)
		return interrors.Errorf("inserting provider id %q for subnet %q %w", subnetInfo.ProviderId, subnetUUID, err)
	}
	...
	if err != nil && !interrors.Is(err, sqlair.ErrNoRows) {
		...
		return interrors.Errorf("retrieving provider network ID %q for subnet %q %w", subnetInfo.ProviderNetworkId, subnetUUID, err)
	} else if interrors.Is(err, sqlair.ErrNoRows) {
		...
		if err := tx.Query(ctx, insertSubnetProviderNetworkIDStmt, providerNetwork).Run(); err != nil {
			...
			return interrors.Errorf("inserting provider network id %q for subnet %q %w", subnetInfo.ProviderNetworkId, subnetUUID, err)
		}
	}
	...
	if err := tx.Query(ctx, insertSubnetProviderNetworkSubnetStmt, providerNetworkSubnet).Run(); err != nil {
		...
		return interrors.Errorf("inserting association between provider network id (%q) %q and subnet %q %w", pnUUIDStr, subnetInfo.ProviderNetworkId, subnetUUID, err)
	}
...
VALUES ($AvailabilityZoneSubnet.*)`, availabilityZoneSubnet)
	...
	for _, az := range subnet.AvailabilityZones {
		...
		if err != nil && !interrors.Is(err, sqlair.ErrNoRows) {
			...
			return interrors.Errorf("retrieving availability zone %q for subnet %q %w", az, subnetUUID, err)
		}
		...
		if interrors.Is(err, sqlair.ErrNoRows) {
			...
			if err != nil {
				return interrors.Errorf("generating UUID for availability zone %q for subnet %q %w", az, subnetUUID, err)
			}
			...
			if err := tx.Query(ctx, insertAvailabilityZoneStmt, availabilityZone).Run(); err != nil {
				...
				return interrors.Errorf("inserting availability zone %q for subnet %q %w", az, subnetUUID, err)
			}
		}
		...
		if err := tx.Query(ctx, insertAvailabilityZoneSubnetStmt, availabilityZoneSubnet).Run(); err != nil {
			...
			return interrors.Errorf("inserting availability zone %q association with subnet %q %w", az, subnetUUID, err)
		}
	}
	return nil
}
...
`
	...
	if err := db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		return interrors.Capture(tx.Query(ctx, s).GetAll(&rows))
	}); interrors.Is(err, sqlair.ErrNoRows) {
		return nil, nil
	} else if err != nil {
		st.logger.Errorf("querying subnets, %v", err)
		return nil, interrors.Errorf("querying subnets %w", err)
	}
...
WHERE  subnet_uuid = $M.id;`
	...
	if err := db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if err != nil {
			...
			return interrors.Errorf("retrieving subnet %q %w", uuid, err)
		}
		return nil
	}); err != nil {
		return nil, interrors.Capture(err)
	...
	if err := db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		for _, cidr := range cidrs {
			...
			if err := tx.Query(ctx, s, sqlair.M{"cidr": cidr}).GetAll(&rows); err != nil {
				...
				return interrors.Errorf("retrieving subnets by CIDR %v %w", cidr, err)
			}
...
WHERE  uuid = $Subnet.uuid;`, subnet)
	...
	if affected != 1 {
		return interrors.Errorf("subnet %q %w", subnet.UUID, coreerrors.NotFound)
	}
...
DELETE FROM subnet WHERE uuid = $Subnet.uuid;`, subnet)
	if err != nil {
		return interrors.Errorf("preparing delete subnet statement %w", err)
	}
...
WHERE  subnet_uuid = $Subnet.uuid;`, subnet, providerNetworkSubnet)
	if err != nil {
		return interrors.Errorf("preparing select provider network statement %w", err)
	}
...
DELETE FROM availability_zone_subnet WHERE subnet_uuid = $Subnet.uuid;`, subnet)
	...
	return db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if delProviderNetworkSubnetAffected, err := outcome.Result().RowsAffected(); err != nil {
			...
			return interrors.Errorf("provider network subnets for subnet %s not found", uuid)
		}
		...
		if delProviderNetworkAffected, err := outcome.Result().RowsAffected(); err != nil {
			...
			return interrors.Errorf("provider network for subnet %s not found", uuid)
		}
		...
		if delProviderSubnetAffected, err := outcome.Result().RowsAffected(); err != nil {
			...
			return interrors.Errorf("provider subnet for subnet %s not found", uuid)
		}
		...
		if delSubnetAffected, err := outcome.Result().RowsAffected(); err != nil {
			...
			return interrors.Errorf("subnet %s not found", uuid)
		}
