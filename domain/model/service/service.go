...
// ModelTypeState represents the state required for determing the type of model
...
//
// If the caller has not prescribed am agent version to use for the model
...
func ModelTypeForCloud(
	...
	if err != nil {
		return coremodel.ModelType(""), fmt.Errorf("determing model type from cloud: %w", err)
	}
