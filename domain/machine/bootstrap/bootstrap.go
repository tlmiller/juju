...
`
		...
		return interrors.Capture(model.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
			...
			if err := tx.Query(ctx, createNodeStmt, createParams).Run(); err != nil {
				return interrors.Errorf("creating net node row for bootstrap machine %q %w", machineId, err)
			}
			if err := tx.Query(ctx, createMachineStmt, createParams).Run(); err != nil {
				return interrors.Errorf("creating machine row for bootstrap machine %q %w", machineId, err)
			}
			return nil
		}))
