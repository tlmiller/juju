...
`
	...
	return func(ctx context.Context, tx *sqlair.TX, poolUUID string, name string, providerType string) error {
		if name == "" {
			return errors.Errorf("storage pool name cannot be empty%w").Add(storageerrors.MissingPoolNameError)
		}
		if providerType == "" {
			return errors.Errorf("storage pool type cannot be empty%w").Add(storageerrors.MissingPoolTypeError)
		}
...
`
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if rowsAffected == 0 {
			return errors.Errorf("storage pool %q not found%w", name).Add(storageerrors.PoolNotFoundError)
		}
		return errors.Errorf("deleting storage pool %w", err)
	})
	return errors.Capture(err)
...
func (st StoragePoolState) ReplaceStoragePool(ctx context.Context, pool domainstorage.StoragePoolDetails) error {
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if err != nil {
			return errors.Errorf("storage pool %q not found%w", pool.Name).Add(storageerrors.PoolNotFoundError)
		}
		poolUUID := dbPool.ID
		if err := storagePoolUpserter(ctx, tx, poolUUID, pool.Name, pool.Provider); err != nil {
