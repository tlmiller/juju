...
func ownerTagFromOwner(owner secrets.Owner) (names.Tag, error) {
	...
	return nil, interrors.Errorf("owner kind %q %w", owner.Kind, coreerrors.NotValid)
}
...
func scopeTagFromAccessScope(scope service.SecretAccessScope) (names.Tag, error) {
	...
	return nil, interrors.Errorf("scope kind %q %w", scope.Kind, coreerrors.NotValid)
}
...
func accessorTagFromAccessor(subject service.SecretAccessor) (names.Tag, error) {
	...
	return nil, interrors.Errorf("subject kind %q %w", subject.Kind, coreerrors.NotValid)
}
...
func (e *exportOperation) Execute(ctx context.Context, model description.Model) error {
	...
	if err != nil {
		return interrors.Errorf("failed to list secrets for export %w", err)
	}

	accessArgsByID, err := e.collateAccess(allSecrets.Access)
	if err != nil {
		return interrors.Errorf("collating secret access args %w", err)
	}

	consumersByID, err := e.collateConsumers(allSecrets.Consumers)
	if err != nil {
		return interrors.Errorf("collating secret consumer args %w", err)
	}

	remoteConsumersByID, err := e.collateRemoteConsumers(allSecrets.RemoteConsumers)
	if err != nil {
		return interrors.Errorf("collating secret remote consumer args %w", err)
	}
	...
	for _, md := range allSecrets.Secrets {
		...
		if err != nil {
			// Should never happen.
			return interrors.Errorf("invalid secret owner %q %w", md.Owner.ID, err)
		}
		...
		for _, rev := range allSecrets.Revisions[md.URI.ID] {
			...
			if ok && len(content) > 0 {
				...
				return interrors.Errorf("missing secret content to export for secret %q", md.URI.ID)
			}
	...
	for _, rs := range allSecrets.RemoteSecrets {
		...
		if err != nil {
			// Should never happen.
			return interrors.Errorf("invalid remote secret accessor %q %w", rs.Accessor.ID, err)
		}
...
func (e *exportOperation) collateConsumers(consumers map[string][]service.ConsumerInfo) (map[string][]description.SecretConsumerArgs, error) {
	...
	for id, infos := range consumers {
		for _, info := range infos {
			...
			if err != nil {
				// Should never happen.
				return nil, interrors.Errorf("invalid secret consumer %q %w", info.Accessor.ID, err)
			}
...
func (e *exportOperation) collateRemoteConsumers(consumers map[string][]service.ConsumerInfo) (map[string][]description.SecretRemoteConsumerArgs, error) {
	...
	for id, infos := range consumers {
		for _, info := range infos {
			...
			if err != nil {
				// Should never happen.
				return nil, interrors.Errorf("invalid remote secret consumer %q %w", info.Accessor.ID, err)
			}
...
func (e *exportOperation) collateAccess(access map[string][]service.SecretAccess) (map[string]map[string]description.SecretAccessArgs, error) {
	...
	for id, perms := range access {
		for _, perm := range perms {
			...
			if err != nil {
				// Should never happen.
				return nil, interrors.Errorf("invalid secret access scope %q %w", perm.Scope.ID, err)
			}
			...
			if err != nil {
				// Should never happen.
				return nil, interrors.Errorf("invalid secret accessor %q %w", perm.Subject.ID, err)
			}
