...
WHERE name=$application.name`, app)
	...
	err = domain.Run(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if errors.Is(err, sqlair.ErrNoRows) {
			return errors.Errorf("application %q not found%w", appName).Add(applicationerrors.ApplicationNotFound)
		}
		if err != nil {
...
WHERE name=$unit.name`, u)
	...
	if errors.Is(err, sqlair.ErrNoRows) {
		return "", errors.Errorf("unit %q not found%w", unitName).Add(applicationerrors.UnitNotFound)
	}
	if err != nil {
...
`, input, secretOwner{}, ownerKindParam)
	...
	err = domain.Run(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if errors.Is(err, sqlair.ErrNoRows) || len(result) == 0 {
			return errors.Errorf("secret %q not found%w", uri).Add(secreterrors.SecretNotFound)
		}
		return errors.Capture(err)
	})
	if err != nil {
...
func (st State) ListSecrets(ctx context.Context, uri *coresecrets.URI,
	...
	if revision != nil {
		revisionNotFoundErr = errors.Errorf(
			"secret revision %d for %s not found%w", *revision, uri).Add(secreterrors.SecretRevisionNotFound)
	}
...
func (st State) GetSecret(ctx context.Context, uri *coresecrets.URI) (*coresecrets.SecretMetadata, error) {
	...
	if len(secrets) == 0 {
		return nil, errors.Errorf("secret %q not found%w", uri).Add(secreterrors.SecretNotFound)
	}
...
WHERE  sm.secret_id = $secretID.id`, secretID{}, secretInfo{})
	...
	if err := db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if errors.Is(err, sqlair.ErrNoRows) {
			return errors.Errorf("rotate policy for %q not found%w", uri).Add(secreterrors.SecretNotFound)
		}
		return errors.Capture(err)
	}); err != nil {
		return coresecrets.RotateNever, errors.Capture(err)
...
WHERE  mso.label = $M.label
	...
	if len(dbSecrets) == 0 {
		return nil, errors.Errorf("secret with label %q not found%w", label).Add(secreterrors.SecretNotFound)
	}
...
`
	...
	if len(dbConsumers) == 0 {
		return nil, errors.Errorf(
			"secret with label %q for unit %q not found%w", label, unitName).Add(secreterrors.SecretNotFound)
	}
...
AND    rev.revision = $secretRevision.revision`
	...
	if len(dbSecretValueRefs) == 0 {
		return nil, nil, errors.Errorf(
			"secret value ref for %q revision %d not found%w", uri, revision).Add(secreterrors.SecretRevisionNotFound)
	}
...
func (st State) lookupSubjectUUID(
	...
	if err != nil {
		if errors.Is(err, sqlair.ErrNoRows) {
			return "", errors.Errorf("%s %q not found%w", subjectTypeID, subjectID).Add(subjectNotFoundError)
		} else {
			subject := subjectID
...
func (st State) lookupScopeUUID(
	...
	if err != nil {
		if errors.Is(err, sqlair.ErrNoRows) {
			return "", errors.Errorf("%s %q not found%w", scopeTypeID, scopeID).Add(scopeNotFoundError)
		} else {
			scope := scopeID
...
DELETE FROM secret_revision WHERE uuid IN ($revisionUUIDs[:])`
	...
	if errors.Is(err, sqlair.ErrNoRows) {
		return nil, errors.Errorf("secret revisions %v not found%w", revs).Add(secreterrors.SecretRevisionNotFound)
	}
	if err != nil {
