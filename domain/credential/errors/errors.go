...
const (
	...
	CredentialModelValidation = interrors.ConstError("credential is not valid for one or more models")

	// NotFound describes an error that occurs when a cloud credential cannot be
	// found.
	NotFound = interrors.ConstError("credential not found")

	// UnknownCloud describes an error that occurs when a credential for cloud
	// not known to the controller is updated.
	UnknownCloud = interrors.ConstError("unknown cloud")
	...
	CredentialNotFound = interrors.ConstError("credential not found")
)
