...
func decodeManifest(manifests []charmManifest) (charm.Manifest, error) {
	...
	for _, base := range manifests {
		...
		if err != nil {
			return charm.Manifest{}, interrors.Errorf("cannot decode channel: %w", err)
		}
...
func decodeManifestChannel(base charmManifest) (charm.Channel, error) {
	...
	if err != nil {
		return charm.Channel{}, interrors.Errorf("cannot decode risk: %w", err)
	}
...
func encodeManifest(id corecharm.ID, manifest charm.Manifest) ([]setCharmManifest, error) {
	...
	for index, base := range manifest.Bases {
		...
		if err != nil {
			return nil, interrors.Errorf("cannot encode risk: %w", err)
		}

		encodedOS, err := encodeManifestOS(base.Name)
		if err != nil {
			return nil, interrors.Errorf("cannot encode OS: %w", err)
		}
		...
		for i, architecture := range base.Architectures {
			...
			if err != nil {
				return nil, interrors.Errorf("cannot encode architecture: %w", err)
			}
			result = append(result, setCharmManifest{
				CharmUUID:      id.String(),
				Index:          index,
...
func encodeManifestOS(os string) (int, error) {
	...
	case "ubuntu":
		return 0, nil
	default:
		return -1, interrors.Errorf("unknown OS %q", os)
	}
}
...
func encodeManifestArchitecture(architecture string) (int, error) {
	...
	case "riscv64":
		return 4, nil
	default:
		return -1, interrors.Errorf("unknown architecture %q", architecture)
	}
}
