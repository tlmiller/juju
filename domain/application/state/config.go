...
func decodeConfig(configs []charmConfig) (charm.Config, error) {
	...
	for _, config := range configs {
		...
		if err != nil {
			return charm.Config{}, interrors.Errorf("cannot decode config type %q: %w", config.Type, err)
		}

		defaultValue, err := decodeConfigDefaultValue(optionType, config.DefaultValue)
		if err != nil {
			return charm.Config{}, interrors.Errorf("cannot decode config default value %v: %w", config.DefaultValue, err)
		}
...
func decodeConfigType(t string) (charm.OptionType, error) {
	...
	default:
		return "", interrors.Errorf("unknown config type %q", t)
	}
}
...
func decodeConfigDefaultValue(t charm.OptionType, value *string) (any, error) {
	...
	default:
		return nil, interrors.Errorf("unknown config type %q", t)
	}
}
...
func encodeConfig(id corecharm.ID, config charm.Config) ([]setCharmConfig, error) {
	...
	for key, option := range config.Options {
		...
		if err != nil {
			return nil, interrors.Errorf("cannot encode config type %q: %w", option.Type, err)
		}

		encodedDefaultValue, err := encodeConfigDefaultValue(option.Default)
		if err != nil {
			return nil, interrors.Errorf("cannot encode config default value %q: %w", option.Default, err)
		}
...
func encodeConfigType(t charm.OptionType) (int, error) {
	...
	case charm.OptionSecret:
		return 4, nil
	default:
		return -1, interrors.Errorf("unknown config type %q", t)
	}
}
...
func encodeConfigDefaultValue(value any) (*string, error) {
	...
	case nil:
		return nil, nil
	default:
		return nil, interrors.Errorf("unknown config default value type %T", value)
	}
}
