...
func decodeMetadata(metadata charmMetadata, args decodeMetadataArgs) (charm.Metadata, error) {
	...
	if metadata.MinJujuVersion != "" {
		...
		if err != nil {
			return charm.Metadata{}, interrors.Errorf("cannot parse min juju version %q: %w", metadata.MinJujuVersion, err)
		}
	}

	runAs, err := decodeRunAs(metadata.RunAs)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("cannot decode run as %q: %w", metadata.RunAs, err)
	}

	provides, requires, peer, err := decodeRelations(args.relations)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("cannot decode relations: %w", err)
	}

	storage, err := decodeStorage(args.storage)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("cannot decode storage: %w", err)
	}

	resources, err := decodeResources(args.resources)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("cannot decode resources: %w", err)
	}

	containers, err := decodeContainers(args.containers)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("cannot decode containers: %w", err)
	}
...
func decodeRunAs(runAs string) (charm.RunAs, error) {
	...
	default:
		return "", interrors.Errorf("unknown run as value %q", runAs)
	}
}
...
func decodeRelations(relations []charmRelation) (map[string]charm.Relation, map[string]charm.Relation, map[string]charm.Relation, error) {
	makeRelation := func(relation charmRelation) (charm.Relation, error) {
		...
		if err != nil {
			return charm.Relation{}, interrors.Errorf("cannot decode relation role %q: %w", relation.Role, err)
		}

		scope, err := decodeRelationScope(relation.Scope)
		if err != nil {
			return charm.Relation{}, interrors.Errorf("cannot decode relation scope %q: %w", relation.Scope, err)
		}
	...
	for _, relation := range relations {
		...
		if err != nil {
			return nil, nil, nil, interrors.Errorf("cannot make relation: %w", err)
		}
		...
		default:
			return nil, nil, nil, interrors.Errorf("unknown relation role %q", relation.Kind)
		}
	}
...
func decodeRelationRole(role string) (charm.RelationRole, error) {
	...
	default:
		return "", interrors.Errorf("unknown relation role %q", role)
	}
}
...
func decodeRelationScope(scope string) (charm.RelationScope, error) {
	...
	default:
		return "", interrors.Errorf("unknown relation scope %q", scope)
	}
}
...
func decodeStorage(storages []charmStorage) (map[string]charm.Storage, error) {
	...
	for _, storage := range storages {
		...
		if err != nil {
			return nil, interrors.Errorf("cannot decode storage type %q: %w", storage.Kind, err)
		}
...
func decodeStorageType(kind string) (charm.StorageType, error) {
	...
	default:
		return "", interrors.Errorf("unknown storage kind %q", kind)
	}
}
...
func decodeResources(resources []charmResource) (map[string]charm.Resource, error) {
	...
	for _, resource := range resources {
		...
		if err != nil {
			return nil, interrors.Errorf("cannot parse resource type %q: %w", resource.Kind, err)
		}
...
func decodeResourceType(kind string) (charm.ResourceType, error) {
	...
	default:
		return "", interrors.Errorf("unknown resource kind %q", kind)
	}
}
...
func encodeMetadata(id corecharm.ID, metadata charm.Metadata, lxdProfile []byte) (setCharmMetadata, error) {
	...
	if err != nil {
		return setCharmMetadata{}, interrors.Errorf("cannot encode run as %q: %w", metadata.RunAs, err)
	}
...
func encodeRunAs(runAs charm.RunAs) (int, error) {
	...
	case charm.RunAsNonRoot:
		return 3, nil
	default:
		return -1, interrors.Errorf("unknown run as value %q", runAs)
	}
}
...
func encodeRelationKind(kind string) (int, error) {
	...
	case relationKindPeers:
		return 2, nil
	default:
		return -1, interrors.Errorf("unknown relation kind %q", kind)
	}
}
...
func encodeRelationRole(role charm.RelationRole) (int, error) {
	...
	case charm.RolePeer:
		return 2, nil
	default:
		return -1, interrors.Errorf("unknown relation role %q", role)
	}
}
...
func encodeRelationScope(scope charm.RelationScope) (int, error) {
	...
	case charm.ScopeContainer:
		return 1, nil
	default:
		return -1, interrors.Errorf("unknown relation scope %q", scope)
	}
}
...
func encodeStorage(id corecharm.ID, storage map[string]charm.Storage) ([]setCharmStorage, []setCharmStorageProperty, error) {
	...
	for key, storage := range storage {
		...
		if err != nil {
			return nil, nil, interrors.Errorf("cannot encode storage type %q: %w", storage.Type, err)
		}
...
func encodeStorageType(kind charm.StorageType) (int, error) {
	...
	case charm.StorageFilesystem:
		return 1, nil
	default:
		return -1, interrors.Errorf("unknown storage kind %q", kind)
	}
}
...
func encodeResources(id corecharm.ID, resources map[string]charm.Resource) ([]setCharmResource, error) {
	...
	for key, resource := range resources {
		...
		if err != nil {
			return nil, interrors.Errorf("cannot encode resource type %q: %w", resource.Type, err)
		}
...
func encodeResourceType(kind charm.ResourceType) (int, error) {
	...
	case charm.ResourceTypeContainerImage:
		return 1, nil
	default:
		return -1, interrors.Errorf("unknown resource kind %q", kind)
	}
}
