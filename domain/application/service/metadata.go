...
func decodeMetadata(metadata charm.Metadata) (internalcharm.Meta, error) {
	...
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode provides relation: %w", err)
	}

	requires, err := decodeMetadataRelation(metadata.Requires)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode requires relation: %w", err)
	}

	peers, err := decodeMetadataRelation(metadata.Peers)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode peers relation: %w", err)
	}

	storage, err := decodeMetadataStorage(metadata.Storage)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode storage: %w", err)
	}

	devices, err := decodeMetadataDevices(metadata.Devices)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode devices: %w", err)
	}

	payloadClasses, err := decodeMetadataPayloadClasses(metadata.PayloadClasses)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode payload classes: %w", err)
	}

	resources, err := decodeMetadataResources(metadata.Resources)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode resources: %w", err)
	}

	containers, err := decodeMetadataContainers(metadata.Containers)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode containers: %w", err)
	}

	assumes, err := decodeMetadataAssumes(metadata.Assumes)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("parse assumes: %w", err)
	}

	charmUser, err := decodeMetadataRunAs(metadata.RunAs)
	if err != nil {
		return internalcharm.Meta{}, interrors.Errorf("decode charm user: %w", err)
	}
...
func decodeMetadataRelation(relations map[string]charm.Relation) (map[string]internalcharm.Relation, error) {
	...
	for k, v := range relations {
		...
		if err != nil {
			return nil, interrors.Errorf("decode role: %w", err)
		}

		scope, err := decodeMetadataScope(v.Scope)
		if err != nil {
			return nil, interrors.Errorf("decode scope: %w", err)
		}
...
func decodeMetadataStorage(storage map[string]charm.Storage) (map[string]internalcharm.Storage, error) {
	...
	for k, v := range storage {
		...
		if err != nil {
			return nil, interrors.Errorf("decode storage type: %w", err)
		}
...
func decodeMetadataResources(resources map[string]charm.Resource) (map[string]resource.Meta, error) {
	...
	for k, v := range resources {
		...
		if err != nil {
			return nil, interrors.Errorf("decode resource type: %w", err)
		}
...
func decodeMetadataContainers(containers map[string]charm.Container) (map[string]internalcharm.Container, error) {
	...
	for k, v := range containers {
		...
		if err != nil {
			return nil, interrors.Errorf("decode mounts: %w", err)
		}
...
func encodeMetadata(metadata *internalcharm.Meta) (charm.Metadata, error) {
	...
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode provides relation: %w", err)
	}

	requires, err := encodeMetadataRelation(metadata.Requires)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode requires relation: %w", err)
	}

	peers, err := encodeMetadataRelation(metadata.Peers)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode peers relation: %w", err)
	}

	err = verifyRelations(metadata, provides, requires, peers)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("verifying charm relations: %w", err)
	}

	storage, err := encodeMetadataStorage(metadata.Storage)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode storage: %w", err)
	}

	devices, err := encodeMetadataDevices(metadata.Devices)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode devices: %w", err)
	}

	payloadClasses, err := encodeMetadataPayloadClasses(metadata.PayloadClasses)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode payload classes: %w", err)
	}

	resources, err := encodeMetadataResources(metadata.Resources)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode resources: %w", err)
	}

	containers, err := encodeMetadataContainers(metadata.Containers)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode containers: %w", err)
	}

	assumes, err := encodeMetadataAssumes(metadata.Assumes)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode assumes: %w", err)
	}

	charmUser, err := encodeMetadataRunAs(metadata.CharmUser)
	if err != nil {
		return charm.Metadata{}, interrors.Errorf("encode charm user: %w", err)
	}
...
func encodeMetadataRelation(relations map[string]internalcharm.Relation) (map[string]charm.Relation, error) {
	...
	for name, v := range relations {
		...
		if err != nil {
			return nil, interrors.Errorf("encode role: %w", err)
		}

		scope, err := encodeMetadataScope(v.Scope)
		if err != nil {
			return nil, interrors.Errorf("encode scope: %w", err)
		}
...
func encodeMetadataStorage(storage map[string]internalcharm.Storage) (map[string]charm.Storage, error) {
	...
	for k, v := range storage {
		...
		if err != nil {
			return nil, interrors.Errorf("encode storage type: %w", err)
		}
...
func encodeMetadataResources(resources map[string]resource.Meta) (map[string]charm.Resource, error) {
	...
	for k, v := range resources {
		...
		if err != nil {
			return nil, interrors.Errorf("encode resource type: %w", err)
		}
...
func encodeMetadataContainers(containers map[string]internalcharm.Container) (map[string]charm.Container, error) {
	...
	for k, v := range containers {
		...
		if err != nil {
			return nil, interrors.Errorf("encode mounts: %w", err)
		}
