...
func (s *CharmService) GetCharm(ctx context.Context, id corecharm.ID) (internalcharm.Charm, charm.CharmOrigin, error) {
	if err := id.Validate(); err != nil {
		return nil, charm.CharmOrigin{}, errors.Errorf("charm id: %w", err)
	}

	resultCharm, resultOrigin, err := s.st.GetCharm(ctx, id)
	if err != nil {
		return nil, charm.CharmOrigin{}, errors.Capture(err)
	}
...
func (s *CharmService) SetCharm(ctx context.Context, args charm.SetCharmArgs) (corecharm.ID, []string, error) {
	...
	if err != nil {
		return "", nil, errors.Errorf("encode charm source: %w", err)
	}

	ch, warnings, err := encodeCharm(args.Charm)
	if err != nil {
		return "", warnings, errors.Errorf("encode charm: %w", err)
	}
...
func encodeCharm(ch internalcharm.Charm) (charm.Charm, []string, error) {
	...
	if err != nil {
		return charm.Charm{}, nil, errors.Errorf("encode metadata: %w", err)
	}

	manifest, warnings, err := encodeManifest(ch.Manifest())
	if err != nil {
		return charm.Charm{}, warnings, errors.Errorf("encode manifest: %w", err)
	}

	actions, err := encodeActions(ch.Actions())
	if err != nil {
		return charm.Charm{}, warnings, errors.Errorf("encode actions: %w", err)
	}

	config, err := encodeConfig(ch.Config())
	if err != nil {
		return charm.Charm{}, warnings, errors.Errorf("encode config: %w", err)
	}
	...
	if lxdProfile, ok := ch.(internalcharm.LXDProfiler); ok && lxdProfile != nil {
		...
		if err != nil {
			return charm.Charm{}, warnings, errors.Errorf("encode lxd profile: %w", err)
		}
	}
