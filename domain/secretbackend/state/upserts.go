...
ON CONFLICT (uuid) DO UPDATE SET
	...
	if database.IsErrConstraintUnique(err) {
		return interrors.Errorf("%w: name %q", backenderrors.AlreadyExists, sb.Name)
	}
	if database.IsErrConstraintTrigger(err) {
		return interrors.Errorf("%w: %q is immutable", backenderrors.Forbidden, sb.ID)
	}
	if err != nil {
		return interrors.Errorf("cannot upsert secret backend %q: %w", sb.Name, err)
	}
	return nil
}
...
ON CONFLICT (backend_uuid) DO UPDATE SET
	...
	if err = tx.Query(ctx, upsertRotationStmt, r).Run(); err != nil {
		return interrors.Errorf("cannot upsert secret backend rotation time for %q: %w", r.ID, err)
	}
	return nil
}
...
ON CONFLICT (backend_uuid, name) DO UPDATE SET
	...
	if err = tx.Query(ctx, clearConfigStmt, sqlair.M{"uuid": id}, namesToKeep).Run(); err != nil {
		return interrors.Errorf("cannot clear secret backend config for %q: %w", id, err)
	}
	for k, v := range cfg {
		...
		if err != nil {
			return interrors.Errorf("cannot upsert secret backend config for %q: %w", id, err)
		}
	}
	return nil
