	...
	err = base.RunAtomic(context.Background(), func(txCtx AtomicContext) error {
		...
		go func() {
			...
			case <-time.After(testing.LongWait):
				secondErr <- interrors.Errorf("failed to start in time")
				return
			}
			...
			err := Run(txCtx, func(ctx context.Context, tx *sqlair.TX) error {
				...
				if atomic.LoadInt64(&called) != 2 {
					return interrors.Errorf("called before first run completed")
				}
		...
		case <-time.After(testing.LongWait):
			return interrors.Errorf("failed to complete first run in time")
		}
		select {
		case err := <-secondErr:
			return err
		case <-time.After(testing.LongWait):
			return interrors.Errorf("failed to complete second run in time")
		}
	})
	c.Assert(err, jc.ErrorIsNil)
