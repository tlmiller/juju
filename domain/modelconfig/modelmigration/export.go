...
func (e *exportOperation) Execute(ctx context.Context, model description.Model) error {
	...
	if config == nil || len(config.AllAttrs()) == 0 {
		return interrors.Errorf("empty model config %w", coreerrors.NotValid)
	}
...
func (noopModelDefaultsProvider) ModelDefaults(context.Context) (modeldefaults.Defaults, error) {
	return modeldefaults.Defaults{}, interrors.Errorf("model defaults %w", coreerrors.NotSupported)
}
