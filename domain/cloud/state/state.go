...
`, dbCloud{})
	...
	if database.IsErrConstraintCheck(err) {
		return errors.Errorf("%w cloud name cannot be empty%w", coreerrors.NotValid).Add(err)
	} else if err != nil {
		return errors.Capture(err)
	}
	return nil
...
WHERE name = $cloudID.name`, cloud)
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if errors.Is(err, sqlair.ErrNoRows) {
			return errors.Errorf("cloud %q %w%w", cloudName, coreerrors.NotFound).Add(err)
		} else if err != nil {
...
WHERE name = $cloudID.name`, cloud)
	...
	if errors.Is(err, sqlair.ErrNoRows) {
		return errors.Errorf("%w %q", clouderrors.NotFound, cloudName)
	} else if err != nil {
		return errors.Errorf("getting cloud %q uuid to set cloud model defaults: %w", cloudName, err)
	}
...
WHERE       cloud_defaults.cloud_uuid = $cloudID.uuid`, cloud)
	if err != nil {
		return errors.Errorf("preparing delete cloud uuid statement %w", err)
	}

	err = tx.Query(ctx, deleteStmt, cloud).Run()
	if err != nil {
		return errors.Errorf("removing previously set cloud %q model defaults: %w", cloudName, err)
	}
...
VALUES ($cloudDefaults.*)`, cloudDefaults{})
	if err != nil {
		return errors.Errorf("preparing insert cloud default statement %w", err)
	}

	err = tx.Query(ctx, insertStmt, dbDefaults).Run()
	if err != nil {
		return errors.Errorf("setting cloud %q model defaults: %w", cloudName, err)
	}
