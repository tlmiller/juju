...
func (st *PermissionState) CreatePermission(ctx context.Context, newPermissionUUID uuid.UUID, spec corepermission.UserAccessSpec) (corepermission.UserAccess, error) {
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if user.Disabled {
			return interrors.Errorf("%w: %q", accesserrors.UserAuthenticationDisabled, user.Name)
		}
...
func AddUserPermission(ctx context.Context, tx *sqlair.TX, spec AddUserPermissionArgs) error {
	...
	if err := spec.Target.ValidateAccess(spec.Access); err != nil {
		return interrors.Errorf("%q for %q %w ", spec.Access, spec.Target.Key, accesserrors.PermissionAccessInvalid)
	}
...
func (st *PermissionState) DeletePermission(ctx context.Context, subject user.Name, target corepermission.ID) error {
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		return interrors.Errorf("delete permission %w", err)
	})
	return interrors.Capture(err)
}
...
func (st *PermissionState) UpdatePermission(ctx context.Context, args access.UpdatePermissionArgs) error {
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if interrors.Is(err, accesserrors.UserNotFound) && !args.Subject.IsLocal() {
		...
		case corepermission.Grant:
			return interrors.Capture(st.grantPermission(ctx, tx, subjectUUID, args))
		case corepermission.Revoke:
			return interrors.Capture(st.revokePermission(ctx, tx, args))
		default:
			return interrors.Capture(interrors.Errorf("change type %q %w", args.Change, coreerrors.NotValid))
		}
	})
	if err != nil {
		return interrors.Capture(err)
...
`
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if interrors.Is(err, sqlair.ErrNoRows) {
			return interrors.Errorf("looking for permissions for %q on %q %w", subject, target.Key, accesserrors.UserNotFound)
		} else if err != nil {
			return interrors.Errorf("getting permission for %q on %q %w", subject.Name(), target.Key, err)
		}
	...
	if err != nil {
		return corepermission.UserAccess{}, interrors.Errorf("for %q on %s %q %w", subject, target.ObjectType, target.Key, err)
	}
...
`
	...
	readStmt, err := st.Prepare(readQuery, user, perm)
	if err != nil {
		return userAccess, interrors.Errorf("preparing select user access level for target statement %w", err)
	}
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if interrors.Is(err, sqlair.ErrNoRows) {
			return interrors.Errorf("%w for %q on %q", accesserrors.AccessNotFound, subject, target.Key)
		} else if err != nil {
			return interrors.Errorf("reading user access level for target %w", err)
		}
	...
	return corepermission.NoAccess, interrors.Errorf("%w for %q on %q", accesserrors.AccessNotFound, subject, target.Key)
}
...
func (st *PermissionState) EnsureExternalUserIfAuthorized(
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if err == nil {
			return nil
		} else if !interrors.Is(err, accesserrors.UserNotFound) {
			return interrors.Errorf("getting user %q", subject)
		}
		// We have a UserNotFound error. Check if everyone@external has permissions
		// on the target.
		baseExternalPerms, err := st.baseExternalAccessForTarget(ctx, tx, target)
		if err != nil {
			return interrors.Errorf("getting everyone@external access %w", err)
		}
	...
	if err != nil {
		return interrors.Errorf("adding external user %q if missing %w", subject, err)
	}
	return nil
}
...
func (st *PermissionState) addExternalUser(ctx context.Context, tx *sqlair.TX, subject user.Name) (user.UUID, error) {
	...
	if interrors.Is(err, accesserrors.UserNotFound) || interrors.Is(err, accesserrors.UserAuthenticationDisabled) {
		return "", interrors.Errorf("%q (should be added on bootstrap) %w", corepermission.EveryoneUserName, accesserrors.UserNotFound)
	}
	...
	if err != nil {
		return "", interrors.Errorf("adding exteranl user %q %w", subject, err)
	}
	return userUUID, nil
}
...
`
	...
	if err != nil {
		return nil, interrors.Errorf("preparing select all access for user query %w", err)
	}

	var externalPerms []dbPermission
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		err = tx.Query(ctx, queryStmt, uName).GetAll(&users, &permissions)
		if interrors.Is(err, sqlair.ErrNoRows) || len(users) == 0 {
			return interrors.Errorf("%q %w", subject, accesserrors.UserNotFound)
		} else if err != nil {
			return interrors.Errorf("getting user with name %q %w", subject, err)
		}
	...
	if users[0].Disabled {
		return nil, interrors.Errorf("%q %w", subject, accesserrors.UserAuthenticationDisabled)
	}

	userAccess, err := st.generateAllUserAccess(users[0], permissions, externalPerms)
	if err != nil {
		return nil, interrors.Errorf("getting permissions for user %q %w", subject, err)
	}
...
`
	...
	if err != nil {
		return nil, interrors.Errorf("preparing select all user access for target statement %w", err)
	}
	var (
		users             []dbPermissionUser
		permissions       []dbPermission
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if interrors.Is(err, sqlair.ErrNoRows) {
			return interrors.Errorf("for %s %q %w", target.ObjectType, target.Key, accesserrors.PermissionNotFound)
		} else if err != nil {
			return interrors.Errorf("collecting permissions on %s %q %w", target.ObjectType, target.Key, err)
		}
	...
	for i, user := range users {
		...
		if err != nil {
			return nil, interrors.Errorf("for %q on %s %q %w", user.Name, target.ObjectType, target.Key, err)
		}
...
func (st *PermissionState) ReadAllAccessForUserAndObjectType(
	...
	default:
		return nil, interrors.Errorf("object type %q %w", objectType, coreerrors.NotValid)
	}
...
`, view)
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if interrors.Is(err, sqlair.ErrNoRows) || len(users) == 0 {
			return interrors.Errorf("for %q on %q %w", subject.Name(), objectType, accesserrors.PermissionNotFound)
		} else if err != nil {
			return interrors.Errorf("getting permissions for %q on %q %w", subject.Name(), objectType, err)
		}
	...
	if err != nil {
		return nil, interrors.Errorf("getting permissions for %q on %q %w", subject.Name(), objectType, err)
	}
...
`
	...
	err = db.Txn(ctx, func(ctx context.Context, tx *sqlair.TX) error {
		...
		if interrors.Is(err, sqlair.ErrNoRows) {
			return interrors.Errorf("for %q on %q %w", key.Owner, key.Cloud, accesserrors.PermissionNotFound)
		} else if err != nil {
			return interrors.Errorf("getting permissions for %q on %q %w", key.Owner, key.Cloud, err)
		}
		return nil
	})
	if err != nil {
...
WHERE  u.removed = false
	...
	if err != nil {
		return result, interrors.Errorf("preparing select getUser query %w", err)
	}
	err = tx.Query(ctx, selectUserStmt, uName).Get(&result)
	if interrors.Is(err, sqlair.ErrNoRows) {
		return result, interrors.Errorf("%q %w", name, accesserrors.UserNotFound)
	} else if err != nil {
		return result, interrors.Errorf("getting user with name %q %w", name, err)
	}
	if result.Disabled {
		return result, interrors.Errorf("%q %w", name, accesserrors.UserAuthenticationDisabled)
	}
	return result, nil
}
...
`
	case corepermission.Offer:
		return nil
	default:
		return interrors.Errorf("object type %q %w", target.ObjectType, coreerrors.NotValid)
	}
	...
	if interrors.Is(err, sqlair.ErrNoRows) {
		return interrors.Errorf("%q %w", target, accesserrors.PermissionTargetInvalid)
	} else if err != nil {
		return interrors.Errorf("verifying %q target exists %w", target, err)
	}
	return nil
}
...
`, inputOut{})
	...
	if err != nil {
		return "", interrors.Errorf("preparing user exist statement %w", err)
	}
...
`
	...
	if err != nil {
		return interrors.Errorf("preparing select existsing user access statement %w", err)
	}
	...
	if interrors.Is(err, sqlair.ErrNoRows) {
		...
		if err != nil {
			return interrors.Errorf("generating new UUID %w", err)
		}
		...
		return interrors.Errorf("getting current access for grant %w", err)
	}
	...
	if aSpec.EqualOrGreaterThan(grantAccess) {
		return interrors.Errorf("user %q already has %q %w", args.Subject, grantAccess, accesserrors.PermissionAccessGreater)
	}
	if err := st.updatePermission(ctx, tx, args.Subject.Name(), args.AccessSpec.Target.Key, grantAccess.String()); err != nil {
		return interrors.Errorf("updating current access during grant %w", err)
	}
	return nil
}
...
func (st *PermissionState) revokePermission(ctx context.Context, tx *sqlair.TX, args access.UpdatePermissionArgs) error {
	...
	if newAccess == corepermission.NoAccess {
		...
		return interrors.Errorf("revoking %q %w", args.AccessSpec.Access, err)
	}
	if err := st.updatePermission(ctx, tx, args.Subject.Name(), args.AccessSpec.Target.Key, newAccess.String()); err != nil {
		return interrors.Errorf("updating current access during revoke %w", err)
	}
	return nil
}
...
`
	...
	if err != nil && !interrors.Is(err, sqlair.ErrNoRows) {
		return interrors.Errorf("deleting permission of %q on %q %w", subject.Name(), target.Key, err)
	}
	return nil
}
...
`
	...
	if err != nil {
		return interrors.Errorf("preparing updatePermission query %w", err)
	}
	...
	if err := tx.Query(ctx, updateQueryStmt, in).Run(); err != nil {
		return interrors.Errorf("updating access on %q for %q to %q %w", grantOn, subjectName, access, err)
	}
	return nil
}
...
AND     u.removed = false
	...
	readStmt, err := st.Prepare(readQuery, user, perm)
	if err != nil {
		return dbPermission{}, interrors.Capture(err)
	}
	err = tx.Query(ctx, readStmt, user, perm).Get(&perm)
	if err != nil && !interrors.Is(err, sqlair.ErrNoRows) {
		return dbPermission{}, interrors.Errorf("getting external user permission on %q %w", target.Key, err)
	}
	return perm, nil
}
...
AND     u.removed = false
	...
	if err != nil && !interrors.Is(err, sqlair.ErrNoRows) {
		return nil, interrors.Errorf("getting external user permission %w", err)
	}
	return perms, nil
}
...
`
	...
	if internaldatabase.IsErrConstraintUnique(err) {
		return interrors.Errorf("%q on %q %w", perm.GrantTo, perm.GrantOn, accesserrors.PermissionAlreadyExists)
	} else if err != nil {
		return interrors.Errorf("adding permission %q for %q on %q %w", perm.AccessType, perm.GrantTo, perm.GrantOn, err)
	}
	return nil
}
