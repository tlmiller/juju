...
func (m *ModelManagerAPI) createNewModelConfig(
	ctx context.Context,
	initalConfig map[string]any,
	modelUUID coremodel.UUID,
) error {
	svc, err := m.modelConfigServiceGetter(modelUUID)
	if err != nil {
		return fmt.Errorf("getting model %q config service: %w", modelUUID, err)
	}

	err = svc.SetModelConfig(ctx, initalConfig)
	if err != nil {
		return err
	}
